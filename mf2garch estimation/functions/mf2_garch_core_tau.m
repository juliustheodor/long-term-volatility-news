function [ e, h, tau, V_m ] = mf2_garch_core(parameters, y, m)
    ...
    for t = 2:m
       ...
       if ((y(t-1) - delta*tau(t-1)) < 0) 
       h(t) = (1-alpha-gamma/2-beta) + (alpha + gamma).*(y(t-1) - delta*tau(t-1)).^2 ./ tau(t-1) + beta .* h(t-1);
       else
       h(t) = (1-alpha-gamma/2-beta) + alpha .* (y(t-1) - delta*tau(t-1)).^2 ./ tau(t-1) + beta .* h(t-1); 
       end
