function [kappa, horizon, forecast, an_vola_forecast, h_forecast, tau_forecast] = mf2_garch_forecasting(y, e, h, tau, m, coeff)
    ...
    kappa = mean((e.^2 - 1).^2) + 1; 
    disp('kappa = ');
    disp(kappa);


	t = length(e); 
	S = 252; % maximum forecast horizon
	%s;
    ...
    if ((lambda_1*1/m*((alpha + gamma/2)*kappa+beta) + lambda_2*((alpha + gamma/2)+beta)) - lambda_1*1/m*((alpha + gamma/2)*kappa+beta) * sum_alpha_beta_deno < 1)
    disp('covariance stationarity condition satisfied');    
    else
    disp('covariance stationarity condition violated');     
    end
    ...
    disp('annualized unconditional volatility (MF)^2 GJR-GARCH-rw-m: ');
    disp((252 * var_m)^0.5);
    ...
    %disp('annualized volatility forecast 1 moth (22 days): ');
    %disp(an_vola_forecast(22));
